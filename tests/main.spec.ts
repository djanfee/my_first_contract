...
describe("main.fc contract tests", () => {
  ...
  it("successfully deposits funds", async () => {
    ...
    expect(balanceRequest.number).toBeGreaterThan(toNano("4.9"));
  });
  ...
  it("successfully withdraws funds on behalf of owner", async () => {
    ...
    const withdrawRequest = await myContract.sendWithdrawalRequest(ownerWallet.getSender(), toNano("0.05"), toNano("1"));
  ...
  it("fails to withdraw funds on behalf of non-owner", async () => {
    ...
    const withdrawRequest = await myContract.sendWithdrawalRequest(senderWallet.getSender(), toNano(0.5), toNano(1));
  ...
  it("fails to withdraw funds because lack of balance", async () => {
    const withdrawRequestResult = await myContract.sendWithdrawalRequest(ownerWallet.getSender(),toNano(0.5),toNano(1));
